...
import com.example.livenativerppg.utils.Native
...
//                    params.whiteBalance = white_balance_auto
    ...
    fun capture(data: ByteArray?, region: Rect) = GlobalScope.async {
        ...
        if (file == null) {
            context.run {
                Toast.makeText(this, "Cannot save a picture", Toast.LENGTH_SHORT).show()
            }
            return@async
        }
        ...
        try {
            ...
            // notify to user
            context.run {
                Toast.makeText(this, "Capture: image saved", Toast.LENGTH_SHORT).show()
            }
