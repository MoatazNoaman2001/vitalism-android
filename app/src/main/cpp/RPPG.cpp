...
bool RPPG::load(jobject listener, JNIEnv *jenv,
                ...
                const bool log, const bool gui , const bool isBP,
                const double Wei,
                const double Hei,
                const float Agg,
                const float Q
    ...
    this->logMode = log;
    this->minFaceSize = Size(min(width, height) * REL_MIN_FACE_SIZE,
                             min(width, height) * REL_MIN_FACE_SIZE);
...
void RPPG::processFrameWithNoFD(Mat &frameRGB, Mat &frameGray,Mat &Mask, int time){
    // Update fps
...
void RPPG::detectFace(Mat &frameRGB, Mat &frameGray) {
    ...
    vector<Rect> boxes;
    classifier.detectMultiScale(frameGray, boxes, 1.1, 2, CASCADE_SCALE_IMAGE, minFaceSize);
...
void RPPG::estimateHeartrate() {
    try {
        ...
        LOGD("error in estimate heart rate: %s", e.msg.c_str());
    }
    // band mask
    ...
    try {
        ...
        bandMask.rowRange(total,  total + 1).setTo(ONE);
    }
...
void RPPG::draw(Mat &frameRGB) {
    ...
    if (!s_f.empty() && !powerSpectrum.empty()) {
        ...
        double drawAreaTlX = box.tl().x;
        double drawAreaTlY = box.tl().y + box.height + 60;
        ...
        p1 = Point(drawAreaTlX,
                   drawAreaTlY + (vmax - powerSpectrum.at<double>(low, 0)) * heightMult);
        for (int i = low + 1; i <= high; i++) {
            p2 = Point(drawAreaTlX + (i - low) * widthMult,
                       drawAreaTlY + (vmax - powerSpectrum.at<double>(i, 0)) * heightMult);
            newPointGenerated(time, drawAreaTlX + (i - low) * widthMult,
                              drawAreaTlY + (vmax - powerSpectrum.at<double>(i, 0)) * heightMult);
            line(frameRGB, p1, p2, RED, 2);
            p1 = p2;
        }
    ...
    if (faceValid) {
        ...
        if (isBP) {
            float ROB = 18.5;
            float ET = 364.5 -1.23  * meanBpm;
            float BSA = 0.007184 * pow(Wei , 0.425) * pow(Hei, 0.725);
            float SV = -6.6+0.25 * (ET- 35) - 0.62 * meanBpm+ 40.4* BSA -0.51  * this->Agg;
            float PP = SV / (0.013 * Wei - 0.007 * Agg - 0.004 * meanBpm  + 1.307);
            float MPP = Q * ROB;

            int sp= (MPP + 3 / 2 * PP);
            int dp= (MPP - PP / 3);
            ss << sp << "/" <<dp << " BP";
            putText(frameRGB, ss.str(), Point(box.tl().x, box.tl().y - 10), FONT_HERSHEY_PLAIN, 2,
                    RED,
                    2);
        } else {
            ss << meanBpm << " bpm";
    ...
    ss.str("");
    ss << fps << " fps";
    putText(frameRGB, ss.str(), Point(box.tl().x, box.br().y + 40), FONT_HERSHEY_PLAIN, 2, GREEN,
            2);

    // Draw corners
    for (int i = 0; i < corners.size(); i++) {
        //circle(frameRGB, corners[i], r, WHITE, -1, 8, 0);
        line(frameRGB, Point(corners[i].x - 5, corners[i].y), Point(corners[i].x + 5, corners[i].y),
             GREEN, 1);
        line(frameRGB, Point(corners[i].x, corners[i].y - 5), Point(corners[i].x, corners[i].y + 5),
             GREEN, 1);
    }
}
